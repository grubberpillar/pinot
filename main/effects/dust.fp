...
vec4 rand_color(float id, float index)
{
    float n = id/(pow(10, index));

    if (n >= 1)
        return palette[int(color_id.r)];
    else
...
// void main()
// {
//     vec4 color_of_pixel = texture2D(texture_sampler, var_texcoord0.xy);
// 
//     if (color_of_pixel.a != 0.0)
//     {
//         vec2 uv = var_texcoord0;
//         color_of_pixel = rand_color(vec2(0.0), uv); // second param gives the pixel-level variation
//     }
// 
...
void main()
{
    ...
    if(color_of_pixel.a != 0.) // when alpha value of the color is not 0 (not transparent) (you can get components of this vector like .r, .b, .g and .a)
    {
        ...
        if (uv.x < 0.5 && uv.y < 0.5) {
            color_of_pixel = rand_color(mod(color_id.a, 10000), 3);
        } else if (uv.x >= 0.5 && uv.y < 0.5) {
            color_of_pixel = rand_color(mod(color_id.a, 1000), 2);
        } else if (uv.x < 0.5 && uv.y >= 0.5) {
            color_of_pixel = rand_color(mod(color_id.a, 100), 1);    
        } else {
            color_of_pixel = rand_color(mod(color_id.a, 10), 0);
        }
    }
